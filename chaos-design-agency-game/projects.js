...
function getClientProfile(project) {
    return project.clientProfile || DEFAULT_CLIENT_PROFILE;
}
...
function handleScopeCreepRequest(change) {
    ...
    changes.forEach(entry => {
        const project = GameState.projects.find(p => p.id === entry.projectId);
        if (!project) return;
        ...
        if (hoursDelta !== 0) {
            ...
            GameState.gameStats.scopeCreepHandled++;
        }
        ...
        if (entry.budgetDelta) {
            ...
            GameState.money += entry.budgetDelta;
        }

        if (entry.teamStress && entry.teamStress !== 0) {
            GameState.team
                .filter(m => m.currentAssignment === project.id)
                .forEach(member => adjustMemberMorale(member, entry.teamStress));
            recalculateTeamMorale();
        }
...
function generateWeeklyClientFeedback() {
    GameState.projects.forEach(project => {
        ...
        if (moraleDelta !== 0) {
            GameState.team
                .filter(m => m.currentAssignment === project.id)
                .forEach(member => adjustMemberMorale(member, moraleDelta));
            recalculateTeamMorale();
        }
    });
}

function cloneClientProfile(profile = DEFAULT_CLIENT_PROFILE) {
    return JSON.parse(JSON.stringify(profile));
}

function buildProjectFromTemplate(template = {}, overrides = {}) {
    const profile = cloneClientProfile(template.clientProfile || DEFAULT_CLIENT_PROFILE);
...
function hydrateProject(project = {}) {
    project.clientProfile = cloneClientProfile(project.clientProfile || DEFAULT_CLIENT_PROFILE);
...
function updateProjects() {
    GameState.projects.forEach(project => {
        if (project.status === 'active' || project.status === 'ok' || project.status === 'warning' || project.status === 'crisis') {
            const assignedMembers = GameState.team.filter(m =>
            ...
            const playerProjects = GameState.projects.filter(p =>
                p.id !== project.id && GameState.team.some(m => m.currentAssignment === p.id && m.id === 'player')
            ).length;
...
function checkProjectDeadlines() {
    GameState.projects.forEach(project => {
        ...
        if (oldStatus !== 'crisis' && status === 'crisis') {
            GameState.gameStats.deadlinesMissed++;
            recordKeyMoment('Project Crisis!', `${project.name} is in crisis`, 'crisis');
        }

        if (project.weeksRemaining < 0 && project.satisfaction < 20 && !project.failureLogged) {
            project.failureLogged = true;
            GameState.gameStats.projectsFailed++;
            recordKeyMoment('Project Failed', `${project.name} collapsed due to low satisfaction and missed deadline`, 'failure');
        }
    });
}
...
function completeProject(projectId) {
    ...
    GameState.portfolio.completedProjects++;
    GameState.portfolio.totalEarnings += payment;

    GameState.gameStats.projectsCompleted++;
    GameState.gameStats.totalSatisfactionPoints += project.satisfaction;
    
    if (project.satisfaction >= 90 && project.weeksRemaining > 0) {
        GameState.gameStats.perfectDeliveries++;
        recordKeyMoment('Perfect Delivery!', `${project.name} completed with ${project.satisfaction}% satisfaction`, 'success');
    } else if (project.satisfaction >= 80) {
        recordKeyMoment('Great Work!', `${project.name} completed successfully`, 'success');
    }
    ...
    celebrateProjectCompletion(project.name);
...
function getTeamMemberStatus(memberId) {
    ...
    if (status === 'burned_out') {
        ...
        const project = GameState.projects.find(p => p.id === member.currentAssignment);
        if (project) {
...
function assignTeamMember(memberId, projectId) {
    ...
    displayGameState();
    highlightTeamMemberCard(memberId);
    saveState();
    return true;
}

function getAvailableTeamMembers() {
    return GameState.team.filter(m => !m.currentAssignment || m.id === 'player');
}
